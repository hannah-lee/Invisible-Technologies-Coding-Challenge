...
function retLocationArr(locations) {
    ...
    if (typeof locations === 'string') {
        if (locations.split(' ', 1)[0] === './weather') { //if the first word of the string is './weather', then remove it 
            var new_str = locations.substr(locations.indexOf("r") + 1);
        ...
        var locations_arr = locations.split(', '); //split the string of locations into an array
...
function logTimeandWeather(location_array) {
    //test to make sure location_array is not empty
    if (location_array.length === 0) {
        throw new Error('Location array cannot be empty');
    }
    var URL = 'http://api.weatherapi.com/v1/current.json?key=c455ae9d79bb4465ab545702200308&q='; //current weather and time API
    var i;
    for (i = 0; i < location_array.length; i++) {
        axios_1["default"].get(URL + location_array[i]) //use axios for simple get request
